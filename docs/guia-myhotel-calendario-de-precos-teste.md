...
description: Guia completo sobre tarifas e disponibilidade no sistema.
slug: /
---
...
Este documento fornece informações detalhadas sobre tarifas, disponibilidade, configurações de calendário e funcionalidades relacionadas ao gerenciamento de tarifas no sistema.
...
Após selecionar o `'tipo de quarto'` do tarifário que deseja consultar, será possível visualizar os preços, restrições e detalhes de disponibilidade.  
...
#### Texto explicativo
Lista de tarifas e pacotes: nesta lista, serão exibidos os tarifários e pacotes criados pelo hotel (para tarifários do tipo pacote, o sistema irá identificá-los com `[P]` antes do nome do tarifário).  

Pesquisar e agrupar por: será possível buscar pelo nome do `'Tarifário'`, do `'Tipo de Quarto'`, pelo nome do `'Canal'` de vendas e pelo nome do `'Nível de Fidelização'` (caso o hotel tenha ativado a solução BeeLoyalty).  
...
#### Texto explicativo
Visualização de 1 mês: permite consultar as informações do mês selecionado.  
...
`'Editar Detalhes da Tarifa / Tipo de Quarto'` para seguir com as atualizações de tarifas.  

Para editar, é possível clicar em `'Editar'`, e, para seguir com as atualizações, usar as opções de:  
